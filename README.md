...
```bash
git clone https://github.com/AmericanPresidentJimmyCarter/stable-diffusion
cd stable-diffusion
pip install -e .
```

Copy the [Stable Diffusion 2 inpainting weights](https://huggingface.co/stabilityai/stable-diffusion-2-inpainting) weights and provide the class instantiation with the checkpoint location via `checkpoint_loc`. The normal SD2 weights should also be supported. It also works with memory efficient flash attention using `xformers.`
...
# PIL images that were generated by sampling
images = itemgetter('images')(extra_data)
images[0].save('foo.png')
```

Diffuse (riff) or inpaint pictures:
...
The sampler is heavily customizable and made to return all parts of the sampling, such as the latent space representations and the conditioning. Refer to the docstrings for more information. You can turn off decoding, production of images, and so on if you desire. You can also alter the k-diffusion sampler's `forward` step via piping in another configuration and extra arguments to the sampler.
